# EXEC Version
# file:a_btc_graph.py date:2021/12/21
# ver 1.06 Tested 2021/
#-------完成実行版 -----
...
# file name
#main = "a_btc5min.xlsx"
#test = "a_btc5min_.xlsx"
#ANA  = "ave_b5min.xlsx"
#test = "ave_b5min_.xlsx"
...
# ---------- file reading  -------------
# 過去の蓄積データ :
df_o =pd.read_excel(pathb + "a_btc5min.xlsx")
...
if cond_TEST: # File 出力　FileSve secでは内容が変わる
    df_out.to_excel(pathb + "a_btc5min_.xlsx" ,index=False)
else:
    df_out.to_excel(pathb + "a_btc5min.xlsx", index=False)
...
now = datetime.now()
####------- 修正--------####
filename = '/Users/suusan/Documents/Python_cron/a_btc5mn'+ now.strftime('%m%d%H') + '.jpg'
